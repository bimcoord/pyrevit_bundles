...
for i in range(1, columns):
    returned_header += ''.join([
        str(selected_size_table.GetColumnHeader(i).Name), '##',
        str(selected_size_table.GetColumnHeader(i).UnitType).replace('UT_',''), '##',
        str(selected_size_table.GetColumnHeader(i).DisplayUnitType).replace('DUT_',''), ';'
        ])
